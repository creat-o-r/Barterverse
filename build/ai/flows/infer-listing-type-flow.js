...
const InferListingTypeInputSchema = genkit_2.z.object({
    name: genkit_2.z.string().describe('The name of the item.'),
    description: genkit_2.z.string().describe('The description of the item.'),
});
const InferListingTypeOutputSchema = genkit_2.z.object({
    inferredListingType: genkit_2.z.enum(['offer', 'want']).describe('The inferred listing type for the item: "offer" or "want".'),
    errorMessage: genkit_2.z.string().optional().describe('An error message if listing type inference failed.'),
});
async function inferListingType(input) {
    return inferListingTypeFlow(input);
}
const prompt = genkit_1.ai.definePrompt({
    name: 'inferListingTypePrompt',
    input: { schema: InferListingTypeInputSchema },
    output: { schema: genkit_2.z.object({ inferredListingType: genkit_2.z.enum(['offer', 'want']) }) },
...
const inferListingTypeFlow = genkit_1.ai.defineFlow({
    ...
    try {
        ...
        if (errorStatus === 401 || errorStatus === 403 || lowerErrorMessage.includes('permission_denied') || lowerErrorMessage.includes('authentication failed')) {
            userMessage = "Authentication error (401/403) with the AI service. Please ensure your GOOGLE_API_KEY in the .env file is correct and active, and that your Google Cloud project has the necessary APIs enabled and billing configured. Defaulted to 'offer'.";
        }
