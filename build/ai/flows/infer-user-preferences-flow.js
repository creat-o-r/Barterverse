...
exports.inferUserPreferences = inferUserPreferences;
/**
...
const UserMotivationEnum = genkit_2.z.enum(['help-others', 'maximize-trades', 'convenience-focused', 'community-building', 'unique-finds']);
const TradeTimingPreferenceEnum = genkit_2.z.enum(['simultaneous', 'staged', 'flexible']);
const MinimumMatchRatingEnum = genkit_2.z.enum(['Low', 'Medium', 'High']);
// Schema for a brief item representation
const ItemBriefSchema = genkit_2.z.object({
    name: genkit_2.z.string(),
    description: genkit_2.z.string().optional(),
...
const UserCurrentPreferencesSchema = genkit_2.z.object({
    motivations: genkit_2.z.array(UserMotivationEnum).optional().describe("The user's explicitly stated trading motivations."),
    ...
    interestedInThirdPartyFulfillment: genkit_2.z.boolean().optional().describe("Whether the user has explicitly stated they are open to 3rd party fulfillments."),
...
const InferUserPreferencesInputSchema = genkit_2.z.object({
    userId: genkit_2.z.string().describe('The ID of the user whose preferences are being inferred.'),
    listedItems: genkit_2.z.array(ItemBriefSchema).optional().describe("A selection of the user's recently listed items (both offers and wants). Include item name, type, category, and a brief description."),
    ...
    simulatedChatSnippets: genkit_2.z.array(genkit_2.z.string()).optional().describe("Example chat snippets (simulated) that might indicate communication style or priorities."),
    engagementNotes: genkit_2.z.array(genkit_2.z.string()).optional().describe("Brief notes on observed or simulated platform engagement patterns (e.g., 'Frequently views 'Electronics' category', 'Often asks detailed questions before trading')."),
    tradesCompleted: genkit_2.z.number().optional().describe("Number of successfully completed trades, indicating experience level."),
});
// This is the schema the AI prompt is asked to output.
const AIPromptOutputSchema = genkit_2.z.object({
    suggestedPreferences: genkit_2.z.object({
        motivations: genkit_2.z.array(UserMotivationEnum).optional(),
        locationPreference: genkit_2.z.object({
            isSensitive: genkit_2.z.boolean(),
            notes: genkit_2.z.string().optional(),
        }).optional(),
        tradeTimingPreference: TradeTimingPreferenceEnum.optional(),
        interestedInThirdPartyFulfillment: genkit_2.z.boolean().optional(),
    ...
    confidence: genkit_2.z.enum(['High', 'Medium', 'Low']).describe("Confidence level in the inferred preferences. This field is REQUIRED."),
    reasoning: genkit_2.z.string().optional().describe("Brief reasoning for the inferred preferences. This field is optional."),
});
// This is the final output schema for the flow, including userId and potential errorMessage
const InferUserPreferencesOutputSchema = genkit_2.z.object({
    userId: genkit_2.z.string(),
    suggestedPreferences: genkit_2.z.object({
        motivations: genkit_2.z.array(UserMotivationEnum).optional(),
        locationPreference: genkit_2.z.object({
            isSensitive: genkit_2.z.boolean(),
    ...
    confidence: genkit_2.z.enum(['High', 'Medium', 'Low']),
    reasoning: genkit_2.z.string().optional(),
    errorMessage: genkit_2.z.string().optional().describe('An error message if preference inference failed.'),
});
async function inferUserPreferences(input) {
...
const inferUserPreferencesFlow = genkit_1.ai.defineFlow({
    ...
    const defaultInferredPreferences = {
    ...
    const processedInput = Object.assign(Object.assign({}, input), { listedItems: (_a = input.listedItems) === null || _a === void 0 ? void 0 : _a.map(item => (Object.assign(Object.assign({}, item), { description: item.description ? item.description.substring(0, 100) + (item.description.length > 100 ? '...' : '') : undefined }))), 
        ...
        currentPreferences: input.currentPreferences
            ? Object.assign(Object.assign({}, input.currentPreferences), { minimumMatchRating: input.currentPreferences.minimumMatchRating || 'Low' }) : { minimumMatchRating: 'Low' } });
    try {
        ...
        const finalSuggestedPreferences = Object.assign({}, defaultInferredPreferences);
        let confidence = 'Low';
        ...
        if (!output) {
            ...
            if (output.suggestedPreferences && typeof output.suggestedPreferences === 'object') {
                finalSuggestedPreferences.motivations = Array.isArray(output.suggestedPreferences.motivations)
                    ? output.suggestedPreferences.motivations.filter((m) => UserMotivationEnum.safeParse(m).success)
                ...
                if (output.suggestedPreferences.locationPreference && typeof output.suggestedPreferences.locationPreference === 'object') {
                    finalSuggestedPreferences.locationPreference = {
                        isSensitive: typeof output.suggestedPreferences.locationPreference.isSensitive === 'boolean' ? output.suggestedPreferences.locationPreference.isSensitive : defaultInferredPreferences.locationPreference.isSensitive,
                        notes: typeof output.suggestedPreferences.locationPreference.notes === 'string' ? output.suggestedPreferences.locationPreference.notes : defaultInferredPreferences.locationPreference.notes,
                    };
                }
                else {
        ...
        return {
            userId: input.userId,
            suggestedPreferences: Object.assign(Object.assign({}, defaultInferredPreferences), { minimumMatchRating: 'Low' }), // Ensure required field
            confidence: 'Low',
