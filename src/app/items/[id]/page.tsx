...
import { MessageSquare, Star, UserCircle, Tag, Info, Repeat, Gift, Search, HelpingHand, Settings, Link2 as LinkIcon } from 'lucide-react';
...
export default async function ItemDetailPage({ params: paramsProp }: { params: { id: string } }) {
  ...
  try {
    // Added optional chaining for dummyUsers[0]?.id for robustness, though unlikely to be the issue itself.
    const isCurrentUserOwner = item.ownerId === dummyUsers[0]?.id; 
    ...
    console.error("[ItemDetailPage] Error during JSX rendering:", e);
    // This will be rendered by Next.js on the server if an error occurs in the try block
    return (
        <div className="container mx-auto px-4 py-8">
            <Card className="border-destructive">
                <CardHeader>
                    <CardTitle className="text-destructive font-headline">Error Displaying Item</CardTitle>
                </CardHeader>
                <CardContent>
                    <p className="font-body">An unexpected error occurred while trying to display this item's details.</p>
                    <p className="font-body mt-2 text-sm">Error message: {e.message}</p>
                    <pre className="mt-2 p-2 bg-muted rounded text-xs whitespace-pre-wrap">{e.stack}</pre>
                </CardContent>
            </Card>
        </div>
    );
