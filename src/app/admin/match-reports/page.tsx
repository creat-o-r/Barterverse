      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="font-body">
            ...
            <br />
            <span className="font-semibold text-destructive-foreground bg-destructive/80 px-2 py-1 rounded-sm inline-block my-1">Important Note:</span> This data is stored in-memory and will be cleared on server restart. For persistent logging, a database solution is required.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {reports.length === 0 ? (
            ...
            <ScrollArea className="h-[calc(100vh-20rem)] w-full border rounded-md">
              <Table>
                <TableHeader className="sticky top-0 bg-card z-10">
                  <TableRow>
                    <TableHead className="w-[180px]">Timestamp</TableHead>
                    <TableHead>For User ID</TableHead>
                    <TableHead>Current Item (Name :: ID)</TableHead>
                  ...
                  {reports.map((report, index) => (
                    <TableRow key={index} className={index % 2 === 0 ? 'bg-muted/30' : ''}>
                      ...
                      <TableCell>
                          ...
                          <div className="text-xs text-muted-foreground">ID: {report.currentItemId}</div>
                      </TableCell>
                      <TableCell>
